-- Qualify columns in create_reservation() that collide with RETURNS TABLE
-- output names such as status and expires_at. PostgreSQL exposes those output
-- names as PL/pgSQL variables, so unqualified column references can become
...
begin
  ...
  select rt.capacity_adults into v_capacity
